...
from api.services.pipeline_runs import create_pipeline_run_job, preflight_pipeline_run_request
from api.services.project_locks import release_project_locks_for_job
...
def create_pipeline_run(
    ...
    return create_pipeline_run_job(
        db,
        payload=payload,
        current_user=current_user,
        submitted_via="hub_web",
    )
...
def submit_prepared_pipeline_run(
    payload: PipelineRunCreateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> Job:
...
@router.post("/preflight", response_model=PipelineRunPreflightResult)
def preflight_pipeline_run(
    payload: PipelineRunCreateRequest,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_user),
) -> PipelineRunPreflightResult:
    return preflight_pipeline_run_request(
        db,
        payload=payload,
        current_user=current_user,
        submitted_via="local_client",
...
def cancel_pipeline_run(
    ...
    if job.status in {"done", "failed"}:
        raise HTTPException(
            ...
            detail=f"Cannot cancel a {job.status} pipeline run",
        )
